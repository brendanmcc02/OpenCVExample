...
//	cv::destroyAllWindows();
	...
	for (int contour_number=0; (contour_number>=0); contour_number=hierarchy[contour_number][0])
	{
		...
		double hu_moments[7];
		HuMoments( contour_moments[contour_number], hu_moments );
		sprintf(output,"Perimeter=%ld, Area=%.0f, BArea=%.0f, CArea=%.0f", contours[contour_number].size(),area,min_bounding_rectangle[contour_number].size.area(),contourArea(hulls[contour_number]));
